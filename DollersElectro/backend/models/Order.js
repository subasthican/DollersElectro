...
const orderSchema = new mongoose.Schema({
  ...
  payment: {
    ...
    status: {
      type: String,
      required: true,
      enum: ['pending', 'processing', 'completed', 'failed', 'refunded', 'cancelled', 'pending_verification', 'verified', 'rejected'],
      default: 'pending'
    },
    ...
    gateway: String,
    amount: Number,
    // Payment Bill Upload (for bank transfer verification)
    billImage: String, // URL to uploaded payment bill/receipt
    billUploadDate: Date, // When customer uploaded the bill
    billVerifiedDate: Date, // When admin verified the bill
    billVerifiedBy: {
      type: mongoose.Schema.Types.ObjectId,
      ref: 'User'
    },
    billRejectionReason: String, // If admin rejects, reason why
  ...
  status: {
    type: String,
    required: true,
    enum: ['pending_payment', 'pending', 'confirmed', 'processing', 'ready', 'shipped', 'delivered', 'completed', 'cancelled', 'refunded'],
