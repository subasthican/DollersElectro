...
// @desc    Get all promo codes (admin/employee)
// @access  Private (admin/employee)
router.get('/', authenticateToken, async (req, res) => {
  // Check role manually - allow both admin and employee
  if (req.userRole !== 'admin' && req.userRole !== 'employee') {
    return res.status(403).json({
      success: false,
      message: `Insufficient permissions. Required role: admin or employee, User role: ${req.userRole}`
    });
  }
  try {
