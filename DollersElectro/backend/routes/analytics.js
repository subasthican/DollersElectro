...
// Note: Some routes allow both admin and employee access
...
router.get('/dashboard', requireRole('admin'), async (req, res) => {
  try {
    ...
    // Get order status counts - group by meaningful categories for pie chart
    // Pending = orders waiting for payment verification or processing
    const pendingOrders = await Order.countDocuments({ 
      status: { $in: ['pending_payment', 'pending', 'under_review'] } 
    });
    // Completed = delivered orders (pickup completed or delivered)
    const completedOrders = await Order.countDocuments({ status: { $in: ['completed', 'delivered'] } });
    const cancelledOrders = await Order.countDocuments({ status: 'cancelled' });
    
    // Calculate total revenue - count from 'confirmed' status onwards (payment verified)
    ...
    const topProducts = await Order.aggregate([
      { $match: { status: { $in: ['confirmed', 'processing', 'ready', 'completed'] } } },
      { $unwind: '$items' },
      {
        $group: {
          _id: '$items.product',
...
// @desc    Get total revenue (admin/employee)
