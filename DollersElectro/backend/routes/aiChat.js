...
router.post('/message', async (req, res) => {
  try {
    ...
    console.log('📨 Received message:', { sessionId: sessionId?.substring(0, 8), message: message?.substring(0, 50) });
    ...
    console.log('✅ Session found, current conversation length:', aiChat.conversation.length);
    ...
    console.log('💬 Added user message, length now:', aiChat.conversation.length);
    
    // Generate AI response using Ollama AI service
    const aiResponse = await generateAIResponse(message, aiChat);
    console.log('🤖 AI response generated:', aiResponse.message?.substring(0, 50));
    aiChat.conversation.push(aiResponse);
    
    console.log('💬 Added AI response, length now:', aiChat.conversation.length);
    ...
    console.log('💾 Saved to database, final conversation length:', aiChat.conversation.length);
