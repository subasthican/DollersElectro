...
const AdminDeliveryManagement: React.FC = () => {
  ...
  const getDeliveryStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'processing': return <ClockIcon className="h-5 w-5 text-blue-700" />;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center">
        <div className="text-center backdrop-blur-2xl bg-white/80 rounded-3xl p-12 shadow-2xl border-2 border-white/60">
          <div className="animate-spin rounded-full h-20 w-20 border-4 border-orange-200 border-t-orange-600 mx-auto"></div>
          ...
          <div className="group backdrop-blur-2xl bg-white/80 hover:bg-white/90 rounded-2xl shadow-xl shadow-green-500/10 hover:shadow-green-500/20 border-2 border-white/60 hover:border-green-200/60 p-6 transition-all duration-300 transform hover:scale-105 cursor-pointer">
            <div className="flex items-center">
              ...
              <div className="ml-4 flex-1">
                ...
                <p className="text-2xl font-bold text-gray-900">{orders.filter(o => o.delivery.status === 'delivered').length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="group backdrop-blur-2xl bg-white/80 hover:bg-white/90 rounded-2xl shadow-xl shadow-indigo-500/10 hover:shadow-indigo-500/20 border-2 border-white/60 hover:border-indigo-200/60 p-6 transition-all duration-300 transform hover:scale-105 cursor-pointer">
            <div className="flex items-center">
              ...
              <div className="ml-4 flex-1">
                ...
                <p className="text-2xl font-bold text-gray-900">{orders.filter(o => ['shipped', 'out_for_delivery'].includes(o.delivery.status)).length}</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
              <div className="flex-1 max-w-lg">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="block w-full pl-12 pr-4 py-3 backdrop-blur-xl bg-white/60 border-2 border-white/40 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-400/50 focus:border-orange-300 focus:bg-white/75 text-gray-900 placeholder-gray-500 shadow-sm transition-all duration-300"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center space-x-4">
                <select
                  ...
                  className="block w-full pl-4 pr-10 py-3 backdrop-blur-xl bg-white/60 border-2 border-white/40 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-400/50 focus:border-orange-300 focus:bg-white/75 text-gray-900 shadow-sm transition-all duration-300 appearance-none cursor-pointer font-medium"
                  style={{
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredOrders.length === 0 ? (
                  ...
                  filteredOrders.map((order) => {
                    // Safe customer name extraction for display
                    ...
                    if (order.customer) {
                      if (typeof order.customer === 'string') {
                        ...
                        customerName = `${(order.customer as any).firstName || ''} ${(order.customer as any).lastName || ''}`.trim() || 'Unknown Customer';
          ...
          <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-base sm:text-lg font-bold bg-gradient-to-r from-gray-700 to-gray-900 bg-clip-text text-transparent">Order Details</h3>
                <button
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {typeof selectedOrder.customer === 'string' ? (
                    <p className="text-sm text-gray-600">Customer ID: {selectedOrder.customer}</p>
                  ) : selectedOrder.customer ? (
                    <>
                      <p className="text-sm text-gray-600">Name: {(selectedOrder.customer as any).firstName || ''} {(selectedOrder.customer as any).lastName || ''}</p>
                      <p className="text-sm text-gray-600">Email: {(selectedOrder.customer as any).email || 'N/A'}</p>
                      <p className="text-sm text-gray-600">Phone: {(selectedOrder.customer as any).phone || 'N/A'}</p>
                    </>
                  ) : (
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Method: {selectedOrder.delivery.method.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</p>
                  <p className="text-sm text-gray-600">Status: {selectedOrder.delivery.status}</p>
                  {selectedOrder.delivery.trackingNumber && (
                    <p className="text-sm text-gray-600">Tracking: {selectedOrder.delivery.trackingNumber}</p>
                  )}
                  {selectedOrder.delivery.carrier && (
                    <p className="text-sm text-gray-600">Carrier: {selectedOrder.delivery.carrier}</p>
                  )}
                </div>
      ...
      {showUpdateModal && selectedOrder && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-base sm:text-lg font-bold bg-gradient-to-r from-gray-700 to-gray-900 bg-clip-text text-transparent">Update Delivery Status</h3>
                <button
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleUpdateDelivery(); }} className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setUpdateForm({ ...updateForm, internalNotes: e.target.value })}
                    rows={3}
                    className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  <button
                    type="button"
                    onClick={() => setShowUpdateModal(false)}
                    className="px-4 py-2 backdrop-blur-xl bg-white/70 border-2 border-white/60 rounded-xl focus:ring-2 transition-all duration-300 shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                  >
                    Cancel
                  </button>
                  <button
